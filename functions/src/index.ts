...
function detectCheatingAttempt(message: string): boolean {
...
async function getSessionHistory(userId: string): Promise<Message[]> {
  try {
    const sessionDoc = await db.collection('ai_sessions').doc(userId).get();
    if (sessionDoc.exists) {
      const data = sessionDoc.data() as AISession;
...
async function updateSessionHistory(userId: string, messages: Message[]): Promise<void> {
  try {
...
export const cyberAssistant = functions.https.onCall(async (data: RequestData, context) => {
  // 1. Verify Firebase Auth
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated to use the AI Assistant'
    );
  }
...
export const analyzeContent = functions.https.onCall(async (
  data: { content: string; type: 'email' | 'sms' | 'url' | 'message' },
  context
) => {
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated to use content analysis'
    );
  }
...
export const clearAISession = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
